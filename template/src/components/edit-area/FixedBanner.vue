<template>
  <div id="fixed-banner">
    ...
    <ErrorMsg :message="validateComponentName" v-if="validateComponentName&&showValidationMsg" :disable="!allowEdit"/>

    <ImageUploader :imgPrefix="'FixedBanner'" :preferSize="'640*220'" :fileName="backgroundImgName" @success="uploadImageSuccess" @remove="removeImg" :disable="!allowEdit"/>
    <ErrorMsg :message="validteBackgroundImg" v-if="validteBackgroundImg&&showValidationMsg"/>

    <TextInput :title="'跳转链接'" :hint="'（必须一些http://或https://开始）'" :placeholder="'点击输入链接'" required v-model="link" :disable="!allowEdit"/>
